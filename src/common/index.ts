...
const changeLanguage = async (
  page: Page,
  target: { selected: string; name: string }
) => {
  const languageButton = await page.$(".language-selector.c-selector");
  const languageSelectedEl = await languageButton?.$(
    "span.c-selector-btn-name"
  );

  const languageSelected = await languageSelectedEl?.evaluate(
  ...
  if (languageSelected !== target.selected) {
    ...
    const languageButtonList = await page.$$(
      "div.c-selector-list-item.genshin"
    );
